...
<head>
    ...
    <style>
        ...
        .login-bg {
            ...
            background-image: radial-gradient(#FFE4E8 2px, transparent 2px),
                linear-gradient(to bottom, #FFF5F7 0%, #FFE4E8 100%);
            background-size: 30px 30px, 100% 100%; z-index: -1;
        }
        .wrapper { width: 100%; max-width: 400px; display: flex; flex-direction: column; align-items: center; }
        .main-title {
            font-family: 'Chewy', cursive; font-size: 3.2rem; color: #2D2D2D;
            text-align: center; line-height: 1.1; margin-bottom: 30px;
            text-shadow: 4px 4px 0px #FF8FA3; word-break: keep-all;
        }
        .login-card {
            width: 100%; background: #FFFFFF; border: 4px solid #2D2D2D;
            border-radius: 40px; padding: 50px 40px;
            box-shadow: 12px 12px 0px #2D2D2D; position: relative;
        }
        .field-label { display: block; color: #2D2D2D; font-size: 1rem; margin-bottom: 10px; padding-left: 5px; }
        .input-box {
            width: 100%; height: 60px; background-color: #FFFFFF;
            border: 3px solid #2D2D2D; border-radius: 20px; padding: 0 20px;
            font-size: 1.1rem; transition: all 0.2s; outline: none;
            margin-bottom: 25px; box-sizing: border-box;
        }
        .input-box:focus { background-color: #FFF5F7; border-color: #FF8FA3; transform: translateY(-2px); }
        .btn-enter {
            width: 100%; height: 65px; background-color: #FF8FA3; color: #FFFFFF;
            border: 3px solid #2D2D2D; border-radius: 22px; font-size: 1.4rem;
            font-family: 'Jua', sans-serif; cursor: pointer;
            box-shadow: 0 6px 0px #2D2D2D; transition: all 0.1s; margin-top: 15px;
            display: flex; align-items: center; justify-content: center;
        }
        .btn-enter:active { transform: translateY(4px); box-shadow: 0 2px 0px #2D2D2D; }
        .find-pw-link {
            display: block; text-align: center; margin-top: 25px;
            color: #9CA3AF; font-size: 0.9rem; text-decoration: none; transition: color 0.2s;
        }
        .find-pw-link:hover { color: #FF5C77; }
        @media (max-width: 480px) {
            ...
            .input-box { height: 55px; font-size: 1rem; margin-bottom: 20px; }
